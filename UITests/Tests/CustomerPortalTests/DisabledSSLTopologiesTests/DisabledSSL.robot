...
Documentation    ${TOPOLOGY_SHORT_NAME_DICT}[${TOPOLOGY}] disabled SSL CRUD test
...
DisabledSSL
    ...
    LaunchServiceDetailsKeywords.Tick 'Disable SSL/TLS' checkbox
    ...
    ConnectToServiceKeywords.Get 'default character set' value without SSL    ${TOPOLOGY}
    ConnectToServiceKeywords.Click on 'Close' button
    ConnectToDatabase.Connect to the current database without SSL    ${TOPOLOGY}
