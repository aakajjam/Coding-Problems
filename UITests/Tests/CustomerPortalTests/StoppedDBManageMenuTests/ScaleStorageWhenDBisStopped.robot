...
Documentation    ${TOPOLOGY_SHORT_NAME_DICT}[${TOPOLOGY}] Scale Storage when the DB is stopped test
...
ScaleStorageWhenDBisStopped
    ...
    ScaleStorageKeywords.Select 'Storage size'    ${STORAGE_ALTERED}
    ...
    ServicesDashboardKeywords.Click on 'Manage' button
    ServicesDashboardKeywords.Click on 'Scale Storage' label
    ScaleStorageKeywords.Verify Storage number is correctly displayed    ${STORAGE_ALTERED}
    ...
    ServiceDetailsKeywords.Verify the Storage number displayed in the service 'Details' tab    ${STORAGE_ALTERED}
